package com.araara.screenapp.util
    ...
    fun updatePermissionStatuses(
        ...
        for (perm in permissions) {
            val granted = when (perm) {
                android.Manifest.permission.READ_MEDIA_IMAGES, android.Manifest.permission.READ_EXTERNAL_STORAGE -> hasStoragePermission(
                    context
                )

                android.Manifest.permission.POST_NOTIFICATIONS -> hasNotificationPermission(context)
                "manage" -> if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) android.os.Environment.isExternalStorageManager() else true
